...
class ExerciseRepository(BaseRepository):
    ...
    async def get_exercise(self, exercise_id: uuid.UUID, user_id: Optional[uuid.UUID] = None) -> ExerciseDetailResponse:
        ...
        exercise_resp = ExerciseDetailResponse.model_validate(exercise)
        ...
        if user_id:
            ...
            # Без user_id упражнение закрыто
            exercise_resp.open = False
    ...
    async def complete_exercise(self, user_id: uuid.UUID, completed_data: CompletedExerciseCreate) -> CompletedExerciseResponse:
        ...
        # Проверяем не выполнено ли уже это упражнение
        already_completed = await self.session.scalar(
            select(CompletedExerciseOrm)
            .where(and_(
                CompletedExerciseOrm.exercise_structure_id == completed_data.exercise_structure_id,
                CompletedExerciseOrm.user_id == user_id
            ))
        )
        if already_completed:
            raise ValueError("This exercise has already been completed")
